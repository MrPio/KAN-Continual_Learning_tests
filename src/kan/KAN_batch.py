...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(),
                 ...
                 sb_trainable=True, device='cpu', seed=0, train_loader=None):
        '''
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        for l in range(self.depth):
            ...
            self.act_fun[l].coef.data = curve2coef(preacts.reshape(batch, spb.size).permute(1, 0),
                                                   ...
                                                   spb.grid, k=spb.k, device=self.device)
    ...
    def train(self, dataset, testset, lr=1, epochs=5, opt="LBFGS", log=1, lamb=0., lamb_l1=1., lamb_entropy=2.,
              lamb_coef=0., lamb_coefdiff=0., update_grid=True,
              grid_update_num=20, loss_fn=None, stop_grid_update_epoch=10, batch=-1, small_mag_threshold=1e-16,
              small_reg_factor=1., metrics=None,
              sglr_avoid=False, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1,
              img_folder='./video', device='cpu'):
        '''
        training
        ...
        -----
            ...
            update_grid : bool
                If True, update grid regularly before stop_grid_update_epoch
            grid_update_num : int
                the number of grid updates before stop_grid_update_epoch
            stop_grid_update_epoch : int
                no grid updates after this training epoch
            batch : int
            ...
            device : str
                device   
            save_fig_freq : int
                save figure every (save_fig_freq) epoch
        ...
        if loss_fn == None:
            loss_fn = loss_fn_eval = lambda x, y: torch.mean((x - y) ** 2) # Substituted with criterion
        else:
            loss_fn = loss_fn_eval = loss_fn

        # FIXME: stop_grid_update_step è diventato stop_grid_update_epoch e quindi noi aggiorniamo al massimo fino alla x epoca
        # dentro ogni epoca, aggiorniamo dui primi "grid_update_freq" batch (di base 2 perché 20/10)
        # ho aumentato grid_update_num da 10 a 20 perché mnist è un dataset più difficile di quelli giocattolo che usano loro per regressione
        grid_update_freq = int(stop_grid_update_epoch / grid_update_num)
        ...
        global train_loss, reg_, train_acc
        ...
        def closure():
            ...
            if sglr_avoid == True:
                ...
                train_loss =  loss_fn(pred[id_], y_batch[id_])
            else:
                # train_loss = loss_fn(pred, dataset['train_label'][train_id].to(device))
                train_loss = criterion(pred, y_batch) #loss_fn(pred, y_batch)
            train_acc = metrics(pred, y_batch)
            if lamb > 0:
        ...
        # for _ in pbar: # FIXME: modifica per gestire i batch
        # FIXME: da dataset intero a dataloader
        # pbar = tqdm(range(steps), desc='description', ncols=100)
        for epoch in range(epochs):
            loss_temp = 0
            train_acc_temp = 0
            pbar = tqdm(dataset, unit="batch", total=len(dataset))
            for batch_idx, (x_batch, y_batch) in enumerate(pbar):
                x_batch, y_batch = x_batch.to(self.device), y_batch.to(self.device)

                # TODO: non mi dovrebbe servire l'id perché uso i batch 
                ...
                loss_temp += train_loss.item()
                train_acc_temp += train_acc.item()

                if batch_idx % log == 0:
                    if loss_fn == None:
                        pbar.set_description(f"[Epoch {epoch}]: train loss: %.2e | reg: %.2e " % (
                            (torch.sqrt(loss_temp) / (batch_idx + 1)), reg_.cpu().detach().numpy()))
                    else:
                        if lamb > 0:
                            pbar.set_description(f"[Epoch {epoch}]: train loss: %.2e | train acc: %.2e | reg: %.2e " % (
                                loss_temp / (batch_idx + 1), train_acc_temp / (batch_idx + 1),
                                reg_.cpu().detach().numpy()))
                        else:
                            pbar.set_description(f"[Epoch {epoch}]: train loss: %.2e | train acc: %.2e | reg: %.2e " % (
                                loss_temp / (batch_idx + 1), train_acc_temp / (batch_idx + 1), 0))

                            # test_loss = loss_fn_eval(self.forward(dataset['test_input'][test_id].to(device)), dataset['test_label'][test_id].to(device))
            test_loss = 0
            test_acc = 0
            pbar = tqdm(testset, unit="batch", total=len(testset))
            for batch_idx_test, (x_batch, y_batch) in enumerate(pbar):
                x_batch, y_batch = x_batch.to(self.device), y_batch.to(self.device)
                pred = self.forward(x_batch)
                test_loss += loss_fn_eval(pred, y_batch)
                test_acc += metrics(pred, y_batch)

                if batch_idx_test % log == 0:
                    if loss_fn == None:
                        pbar.set_description(
                            f"[Epoch {epoch}]: test loss: %.2e" % (torch.sqrt(test_loss.item()) / (batch_idx_test + 1)))
                    else:
                        pbar.set_description(f"[Epoch {epoch}]: test loss: %.2e | test acc: %.2e" % (
                            test_loss.item() / (batch_idx_test + 1), test_acc / (batch_idx_test + 1)))

            if loss_fn == None:
                results['train_loss'].append((torch.sqrt(loss_temp) / (batch_idx + 1)))
                results['test_loss'].append((torch.sqrt(test_loss) / (batch_idx_test + 1)))
            else:
                results['train_loss'].append((loss_temp / batch_idx))
                results['test_loss'].append((test_loss.item() / batch_idx_test))
            if lamb > 0:
                results['reg'].append(reg_.cpu().detach().numpy().item())
            else:
                results['reg'].append(0)
            results['train_accuracy'].append(train_acc_temp / (batch_idx + 1))
            results['test_accuracy'].append(test_acc.cpu().detach().numpy().item() / (batch_idx_test + 1))

            if save_fig and epoch % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title="Step {}".format(batch_idx),
                          beta=beta)
                plt.savefig(img_folder + '/' + str(batch_idx) + '.jpg', bbox_inches='tight', dpi=200)
                plt.close()

        return results

    def distil(self, teacher, dataset, testset, cfg, opt="LBFGS", log=1, lamb=0., lamb_l1=1., lamb_entropy=2.,
               lamb_coef=0., lamb_coefdiff=0., update_grid=True,
               grid_update_num=20, loss_fn=None, lr=1., stop_grid_update_epoch=10, batch=-1, small_mag_threshold=1e-16,
               small_reg_factor=1., metrics=None,
               sglr_avoid=False, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1,
               img_folder='./video', device='cpu'):
        '''
        ...
        -----
            teacher: model (KAN)
                a pre-trained bigger model than this one and whose knowledge will be used as supervision signal
            dataset : dic
                contains dataset['train_input'], dataset['train_label'], dataset['test_input'], dataset['test_label']
            opt : str
                "LBFGS" or "Adam"
            epochs : int
                training epochs
            alpha: float
                [0, 1] - if 0, then the model is only supervised by the teacher; if 1, normal supervision from labels
            temp: float
                > 1: factor by wich logits are divided before softmax. smooths output distribution
            log : int
                logging frequency
            lamb : float
                overall penalty strength
        ...
        if loss_fn == None:
            ...
            loss_fn = loss_fn_eval = loss_fn
        kd_loss_fn = nn.KLDivLoss(log_target=True, reduction='batchmean').cuda()
        ...
        results = {}
        results['train_loss'] = []
        results['train_kd_loss'] = []
        results['test_loss'] = []
        results['test_kd_loss'] = []
        results['reg'] = []
        results['train_accuracy'] = []
        results['test_accuracy'] = []

        global train_loss, loss, kl_loss, reg_, train_acc

        def closure():
            global train_loss, loss, kl_loss, reg_, train_acc
            # pred = self.forward(dataset['train_input'][train_id].to(device))
            pred = self.forward(x_batch)
            pred_t = teacher(x_batch)
            # print(pred_t[0])
            # print(pred[0])
            ...
            kl_loss = kd_loss_fn(nn.functional.log_softmax(pred / temp, dim=-1),
                                 nn.functional.log_softmax(pred_t / temp, dim=-1))
            loss = train_loss * alpha + kl_loss * (1 - alpha)
            train_acc = metrics(pred, y_batch)
            if lamb > 0:
                reg_ = reg(self.acts_scale)
            else:
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (x_batch, y_batch) in enumerate(pbar):
                ...
                loss_temp += train_loss.item()
                kd_loss_temp += kl_loss.item()
                train_acc_temp += train_acc.item()

                if batch_idx % log == 0:
                    if loss_fn == None:
                        pbar.set_description(f"[Epoch {epoch}]: train loss: %.2e | kl loss: %.2e | reg: %.2e " % (
                            (torch.sqrt(loss_temp) / (batch_idx + 1)), kd_loss_temp / (batch_idx + 1),
                            reg_.cpu().detach().numpy()))
                    else:
                        if lamb > 0:
                            pbar.set_description(
                                f"[Epoch {epoch}]: train loss: %.2e | kl loss: %.2e | train acc: %.2e | reg: %.2e " % (
                                    loss_temp / (batch_idx + 1), kd_loss_temp / (batch_idx + 1),
                                    train_acc_temp / (batch_idx + 1), reg_.cpu().detach().numpy()))
                        else:
                            pbar.set_description(
                                f"[Epoch {epoch}]: train loss: %.2e | kl loss: %.2e | train acc: %.2e | reg: %.2e " % (
                                    loss_temp / (batch_idx + 1), kd_loss_temp / (batch_idx + 1),
                                    train_acc_temp / (batch_idx + 1), 0))
            ...
            for batch_idx_test, (x_batch, y_batch) in enumerate(pbar):
                ...
                test_loss += loss_fn_eval(pred, y_batch)
                test_kd_loss += kd_loss_fn(nn.functional.log_softmax(pred / temp, dim=-1),
                                           nn.functional.log_softmax(pred_t / temp, dim=-1))
                test_acc += metrics(pred, y_batch)
                ...
                if batch_idx_test % log == 0:
                    if loss_fn == None:
                        ...
                        pbar.set_description(
                            f"[Epoch {epoch}]: test loss: %.2e | test kd loss: %.2e | test acc: %.2e" % (
                                test_loss.item() / (batch_idx_test + 1), test_kd_loss.item() / (batch_idx_test + 1),
                                test_acc / (batch_idx_test + 1)))
            ...
            if loss_fn == None:
                ...
                results['train_loss'].append((loss_temp / batch_idx))
                results['train_kd_loss'].append((kd_loss_temp) / (batch_idx + 1))
                results['test_loss'].append((test_loss.item() / batch_idx_test))
                results['test_kd_loss'].append((test_kd_loss.item() / batch_idx_test))
            if lamb > 0:
                results['reg'].append(reg_.cpu().detach().numpy().item())
            else:
                results['reg'].append(0)
            results['train_accuracy'].append(train_acc_temp / (batch_idx + 1))
            results['test_accuracy'].append(test_acc.cpu().detach().numpy() / (batch_idx_test + 1))
